...
To see an example script of how to use a `Deployer` class to deploy a contract, check out the deployment section of the quickstart.
...
::: warning zksolc compiler version Starting from version 1.13.14, the zksolc compiler has been enhanced to identify missing libraries. :::
...
::: tip In Typescript projects `--exported-config-object <object name>` argument can be provided optionally. Plugin will try to resolve config by `HardhatUserConfig` type. :::
